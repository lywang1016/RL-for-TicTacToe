...
from framework.player import HumanPlayer, AIPlayer, MinMaxPlayer
...
class Game():
    def __init__(self, r_type, b_type, if_record=False, if_dataset=False, 
        ...
        if r_type == 'human' and b_type == 'ai':    # human vs AI
            self.if_gui = True  
            self.ai_explore_rate = ai_explore_rate                  
            self.gui = GUI()
            self.r_player = HumanPlayer('r')
            self.b_player = AIPlayer('b', self.ai_explore_rate)
        if r_type == 'ai' and b_type == 'human':    # AI vs human
            self.if_gui = True
            self.ai_explore_rate = ai_explore_rate                   
            self.gui = GUI()
            self.r_player = AIPlayer('r', self.ai_explore_rate)
            self.b_player = HumanPlayer('b')
        if r_type == 'ai' and b_type == 'ai':       # AI vs AI
            if self.if_gui:
                self.gui = GUI()
            self.ai_explore_rate = ai_explore_rate 
            self.r_player = AIPlayer('r', self.ai_explore_rate)
            self.b_player = AIPlayer('b', self.ai_explore_rate)
        if r_type == 'human' and b_type == 'minmax':       #  human vs MinMax AI
            self.if_gui = True                   
            self.gui = GUI()
            self.r_player = HumanPlayer('r')
            self.b_player = MinMaxPlayer('b')
        if r_type == 'minmax' and b_type == 'human':       #  MinMax AI vs human 
            self.if_gui = True                   
            self.gui = GUI()
    ...
    def episode(self):
        ...
        if self.r_type == 'human' and self.b_type == 'ai':    # human vs AI
            self.__human_ai_episode()
        if self.r_type == 'ai' and self.b_type == 'human':    # AI vs human
            self.__human_ai_episode()
        if self.r_type == 'ai' and self.b_type == 'ai':       # AI vs AI
            self.__ai_ai_episode()
        if self.r_type == 'human' and self.b_type == 'minmax':       #  human vs MinMax AI
